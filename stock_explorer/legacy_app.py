...
def download_price_histories(tickers: tuple[str, ...], period: str = "5y") -> dict[str, pd.DataFrame]:
    """Lädt Kurshistorien über den konfigurierten Marktdatenanbieter."""
    clean_tickers = tuple(
        dict.fromkeys(clean_ticker(ticker) for ticker in tickers if clean_ticker(ticker))
    )
    clean_tickers = tuple(
        ticker for ticker in clean_tickers
        if re.match(r"^[A-Z0-9][A-Z0-9.\-]*$", ticker)
    )
    return MARKET_PROVIDER.download_price_histories(clean_tickers, period=period)
...
def fetch_ticker_info(ticker: str) -> dict[str, Any]:
    ...
    return MARKET_PROVIDER.get_info(ticker, wanted_keys=wanted_keys)


@st.cache_data(ttl=6 * 60 * 60, show_spinner=False)
def fetch_dividends(ticker: str) -> pd.DataFrame:
    return MARKET_PROVIDER.get_dividends(ticker)
